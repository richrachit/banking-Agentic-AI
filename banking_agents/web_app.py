...
class BankingAppHandler(BaseHTTPRequestHandler):
    ...
    def _dashboard(self, message: str, role: str, name: str) -> None:
        ...
        metric_html = "".join(f"<div style='padding:16px;border-radius:14px;background:linear-gradient(135deg,#eff6ff,#fff);border:1px solid #dbeafe'><div style='color:#64748b;font-weight:600'>{html.escape(label)}</div><div style='font-size:2rem;font-weight:800;color:#1d4ed8'>{count}</div></div>" for label, count in metrics)
        sections.append(f"<section class='card'><div style='display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px'>{metric_html}</div></section>")
        pending_rows = []
        review_rows = []
