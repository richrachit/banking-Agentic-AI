...
class BankingAppHandler(BaseHTTPRequestHandler):
    ...
    def _dashboard(self, message: str, role: str, name: str) -> None:
        ...
        validation_modal = """<style>.message-error{background:#fef2f2!important;color:#b91c1c!important;border-color:#fecaca!important}.agent-status{display:inline-block;padding:5px 9px;border-radius:999px;font-weight:800;font-size:.78rem}.agent-enabled{background:#dcfce7;color:#166534}.agent-disabled{background:#fee2e2;color:#b91c1c}.button-enable{background:linear-gradient(135deg,#059669,#047857)}.button-disable{background:linear-gradient(135deg,#dc2626,#b91c1c)}.chatbot-training{font-size:.86rem;color:#0f766e;font-weight:600}.field-invalid{border-color:#dc2626!important;outline:2px solid rgba(220,38,38,.16)!important}.form-error-overlay{position:fixed;inset:0;display:none;align-items:center;justify-content:center;padding:20px;background:rgba(15,23,42,.52);z-index:20}.form-error-overlay.visible{display:flex}.form-error-dialog{width:min(430px,100%);padding:24px;border-radius:20px;background:#fff;box-shadow:0 24px 70px rgba(15,23,42,.35)}.form-error-dialog h2{margin:0;color:#b91c1c}.form-error-dialog p{color:#475569;line-height:1.5}.form-error-list{margin:10px 0 18px;padding-left:20px;color:#b91c1c;font-weight:600}</style><div id='form-error-modal' class='form-error-overlay' role='alertdialog' aria-modal='true' aria-labelledby='form-error-title'><div class='form-error-dialog'><h2 id='form-error-title'>Complete the required fields</h2><p>Please review the highlighted item(s) before continuing.</p><ul class='form-error-list'></ul><button type='button' id='form-error-close'>Review fields</button></div></div><script>(function(){const modal=document.getElementById('form-error-modal');const list=modal.querySelector('.form-error-list');const close=()=>modal.classList.remove('visible');document.getElementById('form-error-close').addEventListener('click',close);modal.addEventListener('click',event=>{if(event.target===modal)close();});document.querySelectorAll('form').forEach(form=>{if(form.querySelector("input[name='action'][value='logout']"))return;form.noValidate=true;form.addEventListener('input',event=>event.target.classList.remove('field-invalid'));form.addEventListener('change',event=>event.target.classList.remove('field-invalid'));form.addEventListener('submit',event=>{const invalid=[...form.querySelectorAll('input,select,textarea')].filter(control=>!control.disabled&&!control.checkValidity());const action=form.querySelector("input[name='action']")?.value;if(action==='customer_request'){const employment=form.elements.namedItem('employment_type');if(employment&&!employment.value&&!invalid.includes(employment))invalid.push(employment);}if(!invalid.length)return;event.preventDefault();invalid.forEach(control=>control.classList.add('field-invalid'));list.replaceChildren(...invalid.map(control=>{const item=document.createElement('li');const label=control.closest('label');item.textContent=(label?label.childNodes[0].textContent:'Required field').trim()||'Required field';return item;}));modal.classList.add('visible');invalid[0].focus();});});})();</script>"""
        ...
        page = page.replace("</body>", f"{validation_modal}</body>")
        if message.startswith("Input error:"):
            page = page.replace("<div class='message'>", "<div class='message message-error'>")
        self._send(page)
